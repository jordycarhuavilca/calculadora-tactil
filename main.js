...
function changeFirstDefaultCero(value) {
  let trueFalse =
    getTopResultado()[0] === "0" && getTopResultado()[1] !== "." ? true : false;
  if (value === ".") {
    trueFalse = false;
  }
  if (trueFalse) {
...
function setBottonAnswer(value, count, sign) {
  ...
  let newValue = resolver(
    sign,
    getValueA(value, count),
    getValueB(value, count, length())
  );
...
function operando(numeroSignosA, numeroSignosB) {
...
function develop() {
  ...
  if (trueFalse) {
    numeroSignosA = 3;
    numeroSignosB = 2;
    operando(numeroSignosA, numeroSignosB);
  } else {
    numeroSignosA = 2;
    numeroSignosB = 1;
    operando(numeroSignosA, numeroSignosB);
...
function signoEqual() {
  let isTheSame =
    document.querySelector(".calcu__header").className ===
    "calcu__header d-block";
  if (isTheSame) {
    topResultado.textContent = "0";
